...
import { Products, Orders, WishlistItems } from "@/lib/db";

export default function AdminDashboard() {
  const { data: products = [] } = useQuery({ queryKey: ["products"], queryFn: () => Products.list() });
  const { data: orders = [] } = useQuery({ queryKey: ["orders"], queryFn: () => Orders.list("-created_at", 20) });
  const { data: wishlistItems = [] } = useQuery({ queryKey: ["wishlistItems"], queryFn: () => WishlistItems.list() });
    ...
    <div>
        ...
        {stats.map(({ label, value, icon: Icon, to }, i) => (
          <motion.div key={label} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.07 }}>
            <Link to={to} className="block bg-card border border-border/30 p-5 hover:border-border/60 transition-colors duration-300">
              <div className="flex items-start justify-between mb-4"><Icon className="w-4 h-4 text-foreground/30" /></div>
      ...
      <div>
        <div className="flex items-center justify-between mb-5">
          ...
          <Link to="/admin/orders" className="text-[10px] tracking-[0.15em] uppercase text-foreground/25 hover:text-foreground/55 transition-colors">View All →</Link>
        </div>
            ...
            {recentOrders.map((order) => (
              <div key={order.id} className="flex items-center justify-between px-5 py-4">
                <div>
                  ...
                  <p className="text-[10px] text-foreground/30 mt-0.5">{order.customer_phone || order.customer_email || "—"}</p>
                </div>
                <div className="text-right">
                  <p className="text-xs text-foreground/60 font-light">₹{order.total?.toLocaleString()}</p>
                  <span className={`text-[9px] tracking-[0.1em] uppercase border px-2 py-0.5 mt-1 inline-block ${statusColors[order.status] || "text-foreground/30 border-border/30"}`}>{order.status}</span>
                </div>
              </div>
            ))}
          </div>
