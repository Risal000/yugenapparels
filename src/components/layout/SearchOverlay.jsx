        ...
        >
          <div className="max-w-[700px] mx-auto px-6 pt-24">
            <div className="flex items-center justify-between mb-12">
              <span className="text-[10px] tracking-[0.2em] uppercase text-foreground/30">Search</span>
            ...
            <div className="mt-8 space-y-1">
              {loading && <p className="text-xs tracking-[0.1em] text-foreground/30 uppercase py-4">Searching...</p>}
              {!loading && results.length === 0 && query.trim() && (
                <p className="text-xs tracking-[0.1em] text-foreground/30 uppercase py-4">No results found</p>
              )}
              {results.map((product) => (
                <Link
                  key={product.id}
                ...
                >
                  ...
                  <div className="flex-1">
                    <p className="text-sm tracking-wide text-foreground/70 group-hover:text-foreground transition-colors">{product.name}</p>
                    <p className="text-xs text-foreground/30 mt-0.5">₹{product.price?.toLocaleString()}</p>
                  </div>
