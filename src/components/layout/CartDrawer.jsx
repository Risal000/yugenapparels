...
export default function CartDrawer({ isOpen, onClose, cartItems = [] }) {
  ...
  const updateQuantity = async (item, delta) => {
    ...
    if (newQty <= 0) {
      await CartItems.delete(item.id);
    } else {
      await CartItems.update(item.id, { quantity: newQty });
    }
            ...
            <div className="flex-1 overflow-y-auto px-6 py-6">
              {cartItems.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center">
                  ...
                  <p className="text-xs tracking-[0.1em] uppercase text-foreground/30">Your cart is empty</p>
                </div>
              ) : (
                <div className="space-y-6">
                  {cartItems.map((item) => (
                    <motion.div key={item.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex gap-4">
                      {item.product_image && (
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="text-xs font-medium tracking-wide text-foreground/80 truncate">{item.product_name}</h4>
                        {item.size && (
                          <p className="text-[10px] tracking-[0.1em] uppercase text-foreground/30 mt-1">Size: {item.size}</p>
                        )}
                        <p className="text-xs font-medium text-foreground/60 mt-1">₹{item.price?.toLocaleString()}</p>
                        <div className="flex items-center gap-3 mt-3">
                          <button onClick={() => updateQuantity(item, -1)} className="w-6 h-6 border border-border/40 flex items-center justify-center text-foreground/40 hover:text-foreground hover:border-foreground/40 transition-colors">
                            <Minus className="w-3 h-3" />
                          </button>
                          <span className="text-xs text-foreground/60 w-4 text-center">{item.quantity}</span>
                          <button onClick={() => updateQuantity(item, 1)} className="w-6 h-6 border border-border/40 flex items-center justify-center text-foreground/40 hover:text-foreground hover:border-foreground/40 transition-colors">
                            <Plus className="w-3 h-3" />
                          </button>
                          <button onClick={() => removeItem(item)} className="ml-auto text-foreground/20 hover:text-foreground/60 transition-colors">
                            <X className="w-4 h-4" />
                          </button>
                        </div>
                      </div>
            ...
            {cartItems.length > 0 && (
              <div className="border-t border-border/20 px-6 py-5 space-y-4">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] tracking-[0.15em] uppercase text-foreground/40">Total</span>
                  <span className="text-sm font-medium tracking-wide text-foreground">₹{total.toLocaleString()}</span>
                </div>
