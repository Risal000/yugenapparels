...
import { CartItems, WishlistItems } from "@/lib/db";
    ...
    >
      ...
      >
        <div className="relative aspect-[3/4] overflow-hidden bg-card mb-4">
          ...
          >
            <Heart className={`w-4 h-4 transition-colors ${wishlisted ? "fill-white text-white" : "text-white/80"}`} />
          </button>
          <div className={`absolute bottom-0 left-0 right-0 z-10 transition-all duration-400 ease-out ${hovered ? "translate-y-0 opacity-100" : "translate-y-full opacity-0"}`}>
            <button
            ...
            >
              {adding ? <span className="text-black/50">Added ✓</span> : <><ShoppingBag className="w-3 h-3" />Quick Add</>}
            </button>
          </div>
        ...
        <div className="px-0.5 space-y-1.5">
          ...
          <div className="flex items-center gap-2.5">
            <span className="text-[11px] font-medium text-foreground/50">₹{product.price?.toLocaleString()}</span>
            {product.original_price && product.original_price > product.price && (
              <span className="text-[10px] text-foreground/20 line-through">₹{product.original_price?.toLocaleString()}</span>
            )}
          </div>
