...
export default function ProductDetail() {
  ...
  // If a color variant is selected and has images, use those; otherwise fall back to product images
  const baseImages = [product.image_url, product.hover_image_url].filter(Boolean);
  const variantImages = selectedColor
    ? [selectedColor.image_url, selectedColor.hover_image_url].filter(Boolean)
    : [];
        ...
        <div className="lg:grid lg:grid-cols-[1fr_420px] xl:grid-cols-[1fr_480px] lg:gap-16 xl:gap-24">
              ...
              <div className="flex gap-2 mt-3">
                {images.map((img, i) => (
                  <button
                    ...
                    className={`w-16 h-20 lg:w-20 lg:h-24 overflow-hidden transition-all duration-300 ${
                      activeImage === i
                        ? "ring-1 ring-foreground/40 opacity-100"
          ...
          <div className="mt-10 lg:mt-0">
            <div className="lg:sticky lg:top-28">
              ...
              >
                {/* Category */}
                <p className="text-[9px] tracking-[0.35em] uppercase text-foreground/25 mb-5 font-light">
                  {product.category?.replace(/_/g, " ")}
                </p>

                {/* Name */}
                <h1 className="text-2xl lg:text-3xl font-light tracking-[-0.01em] text-foreground/90 leading-tight mb-5">
                  {product.name}
                </h1>

                {/* Price */}
                <div className="flex items-center gap-3 mb-10">
                  <span className="text-base font-light text-foreground/70 tracking-wide">
                    ₹{product.price?.toLocaleString()}
                  </span>
                  {product.original_price && product.original_price > product.price && (
                    <span className="text-sm text-foreground/20 line-through font-light">
                      ₹{product.original_price?.toLocaleString()}
                    </span>
                  )}
                  {product.original_price && product.original_price > product.price && (
                    <span className="text-[9px] tracking-[0.1em] uppercase text-foreground/40 bg-foreground/8 px-2 py-0.5 border border-foreground/10">
                      Sale
                    </span>
                  )}
                </div>
                ...
                {product.sizes?.length > 0 && (
                  <div className="mb-8">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <button className="text-[9px] tracking-[0.15em] uppercase text-foreground/20 hover:text-foreground/40 transition-colors underline underline-offset-2">
                        Size Guide
                      </button>
                    </div>
                    <div className="flex flex-wrap gap-2">
                      {product.sizes.map((size) => (
                        <button
                          ...
                          className={`min-w-[52px] h-11 px-3 text-[10px] tracking-[0.1em] border transition-all duration-250 ${
                            selectedSize === size
                              ? "border-foreground/60 text-foreground bg-foreground/5"
                              : "border-border/30 text-foreground/30 hover:border-foreground/25 hover:text-foreground/55"
                          }`}
                        >
                          {size}
                        </button>
                ...
                <div className="space-y-2.5 mb-10">
                  <button
                    ...
                    className={`w-full py-4 text-[10px] font-medium tracking-[0.2em] uppercase transition-all duration-400 ${
                      addedToCart
                  ...
                  <a
                    href={buildWhatsAppURL(productOrderMessage({
                      name: product.name,
                      price: product.price,
                      size: selectedSize,
                      id: product.id,
                ...
                {product.description && (
                  <div>
                    ...
                    >
                      <p className="text-[9px] tracking-[0.25em] uppercase text-foreground/30 font-light group-hover:text-foreground/50 transition-colors duration-300">
                        Description
                      </p>
                    ...
                    <AnimatePresence>
                      {descOpen && (
                        ...
                        >
                          <p className="text-xs leading-[2] text-foreground/35 font-light pt-4 pb-2">
                            {product.description}
                          </p>
                        </motion.div>
                      )}
                ...
                <div className="mt-8 pt-6 border-t border-foreground/8">
                  <p className="text-[9px] tracking-[0.15em] uppercase text-foreground/20 font-light">
        ...
        {relatedProducts.length > 0 && (
          <div className="mt-28 lg:mt-40 pt-16 border-t border-foreground/8">
            <div className="flex items-center gap-5 mb-12 lg:mb-16">
              ...
              <h2 className="text-[10px] tracking-[0.35em] uppercase text-foreground/30 font-light">
                You May Also Like
              </h2>
            </div>
